...
def is_trusted_host(host: str) -> bool:
    return any(host_matches_domain(host, domain) for domain in DEFAULT_TRUSTED_DOMAINS)
...
def rule_score_url(url: str) -> tuple[float, dict]:
    ...
    reg_domain = ctx.get("registered_domain")
    reg_parts = len(reg_domain.split(".")) if reg_domain else 2
    subdomain_depth = len(host.split(".")) - reg_parts
    if subdomain_depth >= 2 and not re.fullmatch(r"(?:\d{1,3}\.){3}\d{1,3}", host):
        score += 8
